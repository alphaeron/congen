...
class ProgrammedExerciseControllerTest {
    ...
    fun `getAll returns all items for admin`() {
        ...
        whenever(programmedExerciseService.getOwner(EXERCISE_ID_1)).thenReturn(Mono.just("owner1"))
        whenever(programmedExerciseService.getOwner(EXERCISE_ID_2)).thenReturn(Mono.just("owner2"))
        doReturn(Mono.just(true)).whenever(gdprComplianceService).hasUserConsent(any<String>())
