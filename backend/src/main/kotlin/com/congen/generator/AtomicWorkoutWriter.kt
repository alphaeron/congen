 ...
 *
 * This service handles all database writes for workout generation within a single
 * transaction, ensuring data consistency and preventing partial workout data
 * from being written to the database. All operations are performed atomically
 * using the PostgresClient transaction support.
 *
 * @param postgresClient The database client with transaction support
...
class AtomicWorkoutWriter(
    ...
    /**
     * Writes a complete workout generation result to the database within a transaction.
     *
     * @param workoutResult The complete workout generation result to write
     * @return Mono containing the created programmed workout
     */
