--liquibase formatted sql

--changeset John Matty:1 labels:prod,test
--comment: Add exercise Supinated Curl Bar Triceps Push-Down
--rollback SELECT 1

INSERT INTO exercise (name, description, movement_type, is_unilateral, is_upper, is_accessory)
VALUES (
  'Supinated Curl Bar Triceps Push-Down',
  'Engage your core and pin your shoulder blades back.  Keep your elbows close to your body throughout the movement.  Take a slight forward lean at the hips.  Exhale and push the bar down until your arms are fully extended.  Focus on using only your triceps to push the weight down, keeping your shoulders and upper arms still.  Keep your hands in a supinated position throughout the movement.',
  'isolation',
  false,
  true,
  true
);
INSERT INTO exercise_muscle (exercise_name, muscle_name) VALUES
  ('Supinated Curl Bar Triceps Push-Down', 'triceps');
INSERT INTO exercise_equipment (exercise_name, equipment_name) VALUES
  ('Supinated Curl Bar Triceps Push-Down', 'Curl Bar Handle');
INSERT INTO exercise_workout_type (exercise_name, movement_type, workout_type) VALUES
  ('Supinated Curl Bar Triceps Push-Down', 'isolation', 'dynamic_effort'),
  ('Supinated Curl Bar Triceps Push-Down', 'isolation', 'maximal_effort');

