...
2. **Set up and deploy to Kubernetes**
   ...
   ./gradlew :backend:deployToKubernetes -Penvironment=local
   ./gradlew :backend:setupTestPortForward
   ```

3. **Deploy to other environments**
   ```bash
   # Deploy to staging (requires -PremoteRegistry parameter)
   ./gradlew :backend:deployToKubernetes -Penvironment=staging -PremoteRegistry=your-registry.example.com

   # Deploy to production (requires -PremoteRegistry parameter)
   ./gradlew :backend:deployToKubernetes -Penvironment=production -PremoteRegistry=your-registry.example.com

   # Or use Skaffold for development (watches for changes)
   ./gradlew :backend:skaffoldDev
   ```
...
5. **Run tests**
   ...
   ./gradlew :backend:setupTestPortForward  # Run once to setup port forwarding
...
The backend is designed to run on Kubernetes with environment-specific configurations. See the k8s/ directory at the project root for manifests and overlays.

**Note**: Database migrations are automatically generated from Liquibase files during deployment. The `k8s/base/stage-6/migrations-configmap.yaml` file is auto-generated and should not be manually edited.
...
The backend includes comprehensive documentation generation capabilities that automatically create up-to-date API documentation, database schema diagrams, and code documentation.
...
./gradlew :backend:generateApiDocs

# Generate database schema diagram
./gradlew :backend:generateSchemaDot
...
./gradlew :backend:createDocsStructure

# Serve documentation locally
./gradlew :backend:serveDocs
```
...
./gradlew :backend:dokkaGfm

# Generate Javadoc-style documentation
./gradlew :backend:dokkaJavadoc
```
...
./gradlew :backend:checkKubernetesTestEnv

# Cleanup Kubernetes test environment
./gradlew :backend:cleanupKubernetesTestEnv
...
The backend uses Kubernetes-based integration testing:
...
The backend follows Kotlin coding conventions and uses ktlint for code formatting:
...
- `:backend:jib` - Build and push Docker image to remote registry using JIB

#### Skaffold Tasks
- `:backend:skaffoldDev` - Start Skaffold in local development mode (watches for changes)
- `:backend:skaffoldDelete` - Delete Skaffold deployment

#### Configuration Tasks
- `:backend:createMigrationsConfigMap` - Auto-generate ConfigMap for database migrations from Liquibase files
...
./gradlew :backend:deployToKubernetes -Penvironment=staging

# Deploy to production environment (with confirmation prompt)
./gradlew :backend:deployToKubernetes -Penvironment=production
