...
describe('useAutoRefresh', () => {
  ...
  it('should prevent multiple simultaneous refresh calls', async () => {
    ...
    const [firstResult, secondResult] = await Promise.all([firstRefreshPromise, secondRefreshPromise]);
  ...
  it('should handle mutation function errors', async () => {
    ...
    await act(async () => {
      await expect(result.current.executeAndRefresh(mockMutationFunction)).rejects.toThrow('Mutation failed');
    });
