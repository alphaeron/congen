...
export class CycleDiagram {
  ...
  private generateColorPalette(
    ...
    if (segmentCount > baseColors.length) {
      const additionalColors = this.generateAdditionalColors(
        segmentCount - baseColors.length,
        baseColors
      );
  ...
  private render(): void {
    const { nodes, theme = 'default', customColors } = this.props;
    ...
    const innerRadius = maxRadius * 0.82; // Inner radius is 82% of outer
    ...
    nodesWithColors.forEach((node, index) => {
      ...
      const textFitsInside = maxArcWidth > 100 && innerRadius > 100;
      ...
      lines.forEach((line, lineIndex) => {
        ...
        textElement.setAttribute(
          'y',
          (textY + (lineIndex - (lines.length - 1) / 2) * lineHeight).toString()
        );
      ...
      node.details.forEach((detail, detailIndex) => {
        ...
        detailElement.setAttribute(
          'y',
          (textY + halfHeight + 20 * scaleFactor + detailIndex * (12 * scaleFactor)).toString()
        );
  ...
  public updateTextVisibility(showText: boolean): void {
    ...
    const textElements = this.container.querySelectorAll(
      '.cycle-diagram-text, .cycle-diagram-line, .cycle-diagram-detail'
    );
