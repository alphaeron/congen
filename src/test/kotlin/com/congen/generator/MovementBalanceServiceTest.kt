...
class MovementBalanceServiceTest {
    ...
    fun `should calculate pull-to-push ratio correctly`() {
        val state = MovementBalanceService.MovementBalanceState(
    ...
    fun `should detect need for more pull volume`() {
        val state = MovementBalanceService.MovementBalanceState(
            pushVolume = BigDecimal("1000"),
            pullVolume = BigDecimal("1500") // 1.5:1 ratio, less than target 2:1
        )
    ...
    fun `should not need more pull volume when ratio is adequate`() {
        val state = MovementBalanceService.MovementBalanceState(
            pushVolume = BigDecimal("1000"),
            pullVolume = BigDecimal("2500") // 2.5:1 ratio, more than target 2:1
        )
    ...
    fun `should estimate exercise volume correctly for non-accessory`() {
        ...
        val volume = movementBalanceService.estimateExerciseVolume(exercise, false)
    ...
    fun `should estimate exercise volume correctly for accessory`() {
        ...
        val volume = movementBalanceService.estimateExerciseVolume(exercise, true)
    ...
    fun `should score exercise highly when it balances existing movement`() {
        ...
        val score = movementBalanceService.scoreExerciseForBalance(exercise, state, false)
    ...
    fun `should score pull exercise higher when more pull volume needed`() {
        val state = MovementBalanceService.MovementBalanceState(
            pushVolume = BigDecimal("1000"),
            pullVolume = BigDecimal("1500") // Below 2:1 ratio
        )
        val exercise = Exercise(
            name = "Chin-Up",
    ...
    fun `should prioritize exercises for balance correctly`() {
        ...
        val prioritized = movementBalanceService.prioritizeExercisesForBalance(exercises, state, false)
    ...
    fun `should return empty list when no exercises provided`() {
        val state = movementBalanceService.createInitialState()
        val prioritized = movementBalanceService.prioritizeExercisesForBalance(emptyList(), state, false)
    ...
    fun `should handle accessory exercises with different volume estimates`() {
        val state = movementBalanceService.createInitialState()
        val accessoryExercise = Exercise(
            name = "Ab Wheel",
            description = "Ab wheel tool",
            movementType = MovementType.CORE,
            isUnilateral = false,
        ...
        assertEquals(1, updatedState.selectedExercises.size)
        assertEquals(BigDecimal.ZERO, updatedState.pushVolume) // Core not counted as push
        assertEquals(BigDecimal.ZERO, updatedState.pullVolume) // Core not counted as pull
    }
