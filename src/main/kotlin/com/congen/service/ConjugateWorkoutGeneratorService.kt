...
class ConjugateWorkoutGeneratorService(
    ...
    private fun generateWorkoutStages(
        ...
        val primaryExercise = exerciseSelectionService.selectRotatingExercise(
            userId = workout.programId.toInt(), // Using program ID as user ID for now
            targetMuscles = weakMuscles,
            userEquipment = userEquipment,
            preferences = preferences,
            exercises = exerciseSelectionService.filterExercisesByAccessoryStatus(exercises, false), // Only primary exercises (is_accessory = false)
            isAccessory = false, // Primary movements are not accessories
            rotationHistory = rotationHistory
        )

        // Generate primary set schemes for time calculation
        val primarySetSchemes = if (primaryExercise != null) {
        ...
        val secondaryExercise = if (conjugateTemplates.hasSecondaryMovement(dayTemplate.type)) {
            exerciseSelectionService.selectRotatingExercise(
                userId = workout.programId.toInt(),
                targetMuscles = weakMuscles,
                userEquipment = userEquipment,
                preferences = preferences,
                exercises = exerciseSelectionService.filterExercisesExcluding(
        ...
        val secondarySetSchemes = if (secondaryExercise != null) {
        ...
        val numAccessoryExercises = sessionTimeCalculator.calculateNumAccessoryExercisesDynamic(
        ...
        var secondaryMono = if (secondaryExercise != null) {
        ...
        val accessoryMonos = (0 until numAccessoryExercises).map { accessoryIndex ->
            val accessoryExercise = exerciseSelectionService.selectRotatingExercise(
                userId = workout.programId.toInt(),
                targetMuscles = weakMuscles,
                userEquipment = userEquipment,
                preferences = preferences,
                exercises = exerciseSelectionService.filterExercisesByAccessoryStatus(exercises, true), // Only accessory exercises (is_accessory = true)
                    ...
                    .flatMap { accessoryStage ->
                        workoutStageGenerator.createProgrammedExercise(accessoryStage.id, accessoryExercise.name)
                            .flatMap { accessoryProgrammedExercise ->
                                val accessoryScheme = workoutStageGenerator.generatePrilepinBasedScheme(
                                    userId = workout.programId.toInt(),
        ...
        val conditioningMono = if (conjugateTemplates.hasConditioning(dayTemplate.type) && numAccessoryExercises > 0) {
            val conditioningExercise = exerciseSelectionService.selectRotatingExercise(
                userId = workout.programId.toInt(),
                targetMuscles = listOf("full_body"),
                userEquipment = userEquipment,
                preferences = preferences,
                exercises = exerciseSelectionService.filterExercisesByAccessoryStatus(exercises, true), // Use accessory exercises for conditioning (is_accessory = true)
                    ...
                    .flatMap { conditioningStage ->
                        workoutStageGenerator.createProgrammedExercise(conditioningStage.id, conditioningExercise.name)
                            .flatMap { conditioningProgrammedExercise ->
                                val conditioningScheme = workoutStageGenerator.generateAmrapOrEmomScheme(
                                    userId = workout.programId.toInt(),
