...
class WorkoutStageIntegrationTest : BaseIntegrationTest() {
    ...
    fun `should get workout stages by programmed workout id`() {
        ...
        webTestClient.post()
            .uri(
                "/api/v1/workout_stage/?programmedWorkoutId=${workoutResponse.id}&stageTypeId=$secondaryId&position=3&name=Secondary Stage"
            )
            .exchange()
            .expectStatus().isOk()
        webTestClient.get()
    ...
    fun `should delete workout stage`() {
        ...
        val stageResponse =
            webTestClient.post()
                .uri(
                    "/api/v1/workout_stage/?programmedWorkoutId=${workoutResponse.id}&stageTypeId=$stageTypeId&position=20&name=Test Stage"
                )
                .exchange()
                .expectStatus().isOk()
                .expectBody(WorkoutStage::class.java)
